...
LOCAL_VERSION_FILE="./.gemini/get-shit-done/VERSION"
LOCAL_MARKER_FILE="./.gemini/get-shit-done/workflows/update.md"
...
(Paths are relative to your install location: `./.gemini/` for global, `./.gemini/` for local)
...
The SessionStart hook (`gsd-check-update.js`) always writes to `./.gemini/cache/` via `os.homedir()` regardless of install type, so both paths must be cleared to prevent stale update indicators.
</step>
